...
class PlannerAgent:
    ...
    def plan(self, user_id: int, session_id: int, analysis: AnalysisResult, chat_window_service: ChatWindowService) -> ExecutionPlan:
        if self.llm_client is None:
            return self._fallback_plan(analysis)

        try:
            plan_data = self._plan_by_llm(user_id, session_id, analysis, chat_window_service)
            res = self._sanitize_plan(plan_data, analysis)
            return res
        except Exception as e:
            print(f"plan error:{e}")
            return self._fallback_plan(analysis)

    def _plan_by_llm(self, user_id: int, session_id: int, analysis: AnalysisResult, chat_window_service: ChatWindowService) -> Dict[str, Any]:
...
你的任务是根据用户画像、历史对话、用户现在问题做子问题拆解和知识路由，不直接回答用户问题。
结合上下文发现用户现在问题不可分时，只返回一个子问题即用户原问题

系统只有三四类知识源：
1. medical_graph：医疗知识图谱
2. policy_rag：政策类向量检索只适用于policy_consult
3. memory：长期记忆向量检索（必须和其他知识源一起出现，产生2个及以上的子问题）
4. none：与医疗无关，不需要检索，直接回答

知识源可以复用，子问题可以拆为不同知识源，也可拆为同一知识源。
旦当子问题中knowledge_route为memory时，必须多设计一个其他知识源的子问题兜底
输出 JSON，不要输出任何额外文字。
"""
...
{{
  ...
  "tasks": [
    {{
      ...
      "knowledge_route": "medical_graph 或 policy_rag 或 memory 或 none",
      "question": "子问题1",
    ...
    def _sanitize_plan(self, plan_data: Dict[str, Any], analysis: AnalysisResult) -> ExecutionPlan:
        ...
        for idx, item in enumerate(plan_data.get("tasks", []), start=1):
            try:
                ...
                print(f"[sanitize] task {idx} 构造失败: {e}")
                raise
